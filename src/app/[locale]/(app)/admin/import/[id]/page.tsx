...
export default async function ImportReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!role) notFound();
  const [{ data: job }, { data: rows, error }] = await Promise.all([
  ...
  if (!job) notFound();
  const grouped = (rows ?? []).reduce<Record<string, number>>((acc, row) => { acc[row.status] = (acc[row.status] ?? 0) + 1; return acc; }, {});
  return <div className="mx-auto max-w-7xl px-4 py-8 md:px-6">
    ...
    {error ? <p role="alert" className="mt-4">{t("loadError")}</p> : <>
      <div className="mt-6 flex flex-wrap gap-2">{Object.entries(grouped).map(([status, count]) => <span key={status} className="rounded-full border px-3 py-1.5 text-xs">{status}: {count}</span>)}</div>
      <div className="mt-6 overflow-x-auto rounded-2xl border"><table className="w-full min-w-[800px] text-sm"><thead className="bg-[var(--color-surface-muted,#f6f0e8)] text-start"><tr><th className="p-3">{t("row")}</th><th className="p-3">{t("status")}</th><th className="p-3">{t("issues")}</th><th className="p-3">{t("preview")}</th><th className="p-3">{t("match")}</th></tr></thead><tbody>{(rows ?? []).map(row => <tr key={row.id} className="border-t align-top"><td className="p-3 font-mono">{row.row_number}</td><td className="p-3"><span className="rounded-full border px-2 py-1 text-xs">{row.status}</span></td><td className="p-3 text-xs text-[var(--color-muted-text)]">{row.issues?.join(" · ") || "—"}</td><td className="max-w-xl p-3"><pre className="whitespace-pre-wrap break-words text-xs">{JSON.stringify(row.raw_data, null, 2)}</pre></td><td className="p-3 font-mono text-xs">{row.matched_existing_id || "—"}</td></tr>)}</tbody></table></div>
    </>}
