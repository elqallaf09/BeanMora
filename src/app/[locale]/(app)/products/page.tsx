...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  let products = (data ?? []).map(p => ({ ...p, roaster: singleRelation(p.roaster), lot: singleRelation(p.lot) }));
  if (country) products = products.filter(p => p.roaster?.country === country);
  if (currency) products = products.filter(p => (p.prices ?? []).some(price => price.currency === currency));
  return <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
    <header className="mb-6">
      ...
      <Link href="/products/compare" className="mt-3 inline-flex rounded-full border px-4 py-2 text-xs font-semibold">{t("comparePrices")}</Link>
    </header>
    <form className="mb-6 grid gap-2 sm:grid-cols-2 lg:grid-cols-6">
      <input name="q" defaultValue={q} placeholder={t("search")} aria-label={t("search")} className="h-11 rounded-xl border bg-[var(--color-surface)] px-3 lg:col-span-2" />
      <select name="method" defaultValue={method} aria-label={t("allMethods")} className="h-11 rounded-xl border bg-[var(--color-surface)] px-3">
      ...
      <select name="country" defaultValue={country} aria-label={t("allCountries")} className="h-11 rounded-xl border bg-[var(--color-surface)] px-3">
        <option value="">{t("allCountries")}</option>
        {["Kuwait", "Saudi Arabia", "United Arab Emirates", "Qatar", "Bahrain", "Oman"].map(c => <option key={c} value={c}>{t(`countries.${c}`)}</option>)}
      </select>
      <select name="currency" defaultValue={currency} aria-label={t("allCurrencies")} className="h-11 rounded-xl border bg-[var(--color-surface)] px-3">
    ...
    {error ? <RichEmptyState icon={Coffee} title={t("error")} /> : products.length === 0 ? <RichEmptyState icon={Coffee} title={t("empty")} description={t("emptyHint")} /> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{products.map(p => {
      const sortedPrices = [...(p.prices ?? [])].sort((a, b) => new Date(b.recorded_at).getTime() - new Date(a.recorded_at).getTime());
      const latestPrice = sortedPrices[0];
      const comparable = latestPrice && Number(p.weight_grams) > 0 ? Number(latestPrice.price) * 100 / Number(p.weight_grams) : null;
      ...
      return <article key={p.id} className="rounded-3xl border border-[var(--color-border,#ece1d3)] bg-[var(--color-surface)] p-5 shadow-sm">
        ...
        <div className="mt-4 flex items-end justify-between gap-3"><div>{latestPrice ? <p className="text-lg font-bold">{Number(latestPrice.price).toLocaleString(locale)} {latestPrice.currency}</p> : <p className="text-xs text-[var(--color-muted-text)]">{t("priceUnknown")}</p>}<p className="text-xs text-[var(--color-muted-text)]">{p.weight_grams ? `${p.weight_grams} g` : ""}</p>{latestPrice && comparable !== null ? <p className="mt-1 text-xs font-semibold text-[var(--color-copper)]">{comparable.toLocaleString(locale, { maximumFractionDigits: 3 })} {latestPrice.currency} / 100g</p> : null}</div>
          {p.purchase_url ? <a href={p.purchase_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-full border px-3 py-2 text-xs font-semibold">{t("buy")}<ExternalLink className="h-3.5 w-3.5" /></a> : null}
        </div>
