...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{products.map((p:any) => {
      const sortedPrices = [...(p.prices ?? [])].sort((a:any,b:any) => new Date(b.recorded_at).getTime()-new Date(a.recorded_at).getTime());
      const latestPrice = sortedPrices[0];
      const comparable = latestPrice && p.weight_grams ? Number(latestPrice.price) * 100 / Number(p.weight_grams) : null;
