...
export default async function ComparePricesPage() {
  ...
  const t = await getTranslations("priceCompare");
  const all = await getTranslations();
  const supabase = await createClient();
  const { data, error } = await supabase.from("roasted_products").select("id,slug,name_ar,name_en,weight_grams,status,roaster:roasters(name_ar,name_en,country),prices:product_prices(price,currency,recorded_at)").eq("requires_review", false).not("weight_grams", "is", null).limit(100);
