...
// Normalize object/array relation shapes while the generated database types are pending.
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!p) notFound();
  const roaster = singleRelation(p.roaster);
  const lot = singleRelation(p.lot);
  ...
  const prices = [...(p.prices ?? [])].sort((a, b) => new Date(b.recorded_at).getTime() - new Date(a.recorded_at).getTime());
  const availability = [...(p.availability ?? [])].sort((a, b) => new Date(b.recorded_at).getTime() - new Date(a.recorded_at).getTime());
  return <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6">
    <header className="rounded-3xl border bg-[var(--color-surface)] p-6">
      <p className="text-xs font-semibold text-[var(--color-copper)]">{roaster ? localizedField(roaster,"name",locale) : ""}</p>
      ...
      <div className="mt-4 flex flex-wrap gap-2">{[p.status,p.roast_level,p.weight_grams? `${p.weight_grams} g`:null].filter(Boolean).map(x=><span key={x} className="rounded-full border px-3 py-1 text-xs">{x}</span>)}</div>
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-2">
      <section className="rounded-3xl border bg-[var(--color-surface)] p-5"><h2 className="text-lg font-bold">{t("origin")}</h2>
        <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">{Object.entries({[t("country")]:lot?.origin_country,[t("region")]:lot?.origin_region,[t("farm")]:lot?.farm,[t("producer")]:lot?.producer,[t("varietal")]:lot?.varietal,[t("process")]:lot?.process,[t("harvest")]:lot?.harvest_season}).filter(([,v])=>v).map(([k,v])=><div key={k}><dt className="text-xs text-[var(--color-muted-text)]">{k}</dt><dd className="font-medium">{String(v)}</dd></div>)}</dl>
      </section>
      <section className="rounded-3xl border bg-[var(--color-surface)] p-5"><h2 className="text-lg font-bold">{t("priceHistory")}</h2>
        <div className="mt-4 space-y-3">{prices.length?prices.slice(0,10).map((x,i)=><div key={i} className="flex justify-between border-b pb-2 text-sm"><strong>{Number(x.price).toLocaleString(locale)} {x.currency}</strong><span className="text-[var(--color-muted-text)]">{new Date(x.recorded_at).toLocaleDateString(locale)}</span></div>):<p className="text-sm text-[var(--color-muted-text)]">{t("noPrices")}</p>}</div>
      </section>
      <section className="rounded-3xl border bg-[var(--color-surface)] p-5 lg:col-span-2"><h2 className="text-lg font-bold">{t("availabilityHistory")}</h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">{availability.length?availability.slice(0,10).map((x,i)=><div key={i} className="rounded-2xl border p-3 text-sm"><div className="flex justify-between"><strong>{x.status}</strong><span className="text-xs text-[var(--color-muted-text)]">{new Date(x.recorded_at).toLocaleDateString(locale)}</span></div>{x.note?<p className="mt-1 text-[var(--color-muted-text)]">{x.note}</p>:null}</div>):<p className="text-sm text-[var(--color-muted-text)]">{t("noAvailability")}</p>}</div>
      </section>
    </div>
